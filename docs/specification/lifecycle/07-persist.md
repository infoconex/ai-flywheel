---
title: "Stage 7: Persist"
document_type: "lifecycle-stage"
section: "specification"
status: "canonical"
canonical: true
order: 7
summary: "Validated and authorized learning is stored in a durable operational asset so it can survive the current execution."
stage_number: 7
stage_id: "persist"
last_reviewed: "2026-07-16"
tags:
  - "lifecycle"
---

# Stage 7: Persist

Validated and authorized learning is stored in a durable operational asset so it can survive the current execution.

## Purpose

A lesson does not become operational learning merely because it was observed or discussed.

Persistence changes something that future executions can actually use.

Possible persistent assets include:

- deterministic capabilities,
- code changes,
- SOP updates,
- validation rules,
- durable reasoning guidance,
- reusable knowledge,
- and approved Governance Policy changes.

## Persistence Must Match the Learning

The selected asset should reflect the classification made earlier in the lifecycle.

A deterministic bug should normally be corrected in deterministic implementation. A procedural weakness should normally change the SOP. A contextual lesson that still requires judgment should remain available as reasoning knowledge rather than being forced into brittle code.

## Persistence Requires Authority

A validated change may only be persisted when the system is authorized to make that change.

Changes that expand delegated authority or modify protected governance require human authorization.

## Persistent Does Not Mean Permanent

Future evidence may justify refining, replacing, rolling back, invalidating, or moving a persistent asset to another operating mechanism.

Persisted learning remains subject to future Flywheel cycles.

## Stage Navigation

- [Lifecycle Index](README.md)
- **Previous Stage:** [Stage 6: Validate](06-validate.md)
- **Next Stage:** [Stage 8: Reuse](08-reuse.md)

## Related Documents

- [Principle 7: Learning Must Change a Persistent Operational Asset](../principles/07-persistent-learning.md)
- [Principle 6: Failure Determines Where the System Evolves](../principles/06-evolution-routing.md)
